...
class DinoEncoder(nn.Module):
    def __init__(self, device='cuda'):
        ...
        self.model = transformers.AutoModel.from_pretrained('facebook/dinov2-base').to(device)
        self.model.eval()
...
class Recording:
    ...
    def __init__(self, path: str):
        ...
        self.video_files = [f.path for f in os.scandir(self.path) if f.path.endswith(".mp4")]
        self.video_files = sorted(self.video_files, key=lambda x: int(x.split('_')[-1].split('.')[0]))

        self.encoding_files = [f.path for f in os.scandir(self.path) if f.path.endswith(".h5")]
    ...
    def infer(self, model: nn.Module, dataset_name : str, behaviors: str, seq_len: int, device=None):
        for file_path in self.encoding_files:
            output_file = file_path.replace('_cls.h5', '_'+dataset_name+'_outputs.csv')
...
class Camera:
    ...
    def get_new_recording_dir(self):
        date_dir = os.path.join(self.project.recordings_dir, datetime.now().strftime('%Y%m%d'))

        if not os.path.exists(date_dir):
            os.mkdir(date_dir)

        cam_session = self.name + '-' + datetime.now().strftime('%I%M%S-%p')
        session_dir = os.path.join(date_dir, cam_session)
        ...
        # If this happens some bad stuff occurred!
        print("Identical recording session directory created.")
        return None
    ...
    def start_recording(self, destination: str, segment_time: int):
        ...
        command = [
            'ffmpeg', '-loglevel', 'panic', '-rtsp_transport', 'tcp', '-i', str(self.rtsp_url),
...
class Project:
    def __init__(self, path: str):
        ...
        for cam_path in [f.path for f in os.scandir(self.cameras_dir) if f.is_dir()]:
            ...
            self.cameras.append(Camera(config, self))
        
        # Build out list of datasets
        self.datasets = {}
        for dataset_path in [f.path for f in os.scandir(self.datasets_dir) if f.is_dir()]:
    ...
    def infer_recordings(self, device=None):
        ...
        state_dict = model
        model = classifier_head.classifier(in_features=768, out_features=len(config['behaviors']), seq_len=config['seq_len'])
        ...
        for day in self.recordings:
            for recording in self.recordings[day]:
                self.recordings[day][recording].infer(model, "JonesLabModel", config['behaviors'], 31, "cuda")
    ...
    def convert_instances(self, insts: list, seq_len: int, behaviors: list[str]) -> tuple[list[torch.tensor], list[int]]:
        seqs = []
        labels = []
    ...
    def load_dataset(self, name: str, seed=42, split=0.2, seq_len=15) -> tuple[Dataset, Dataset]:
        ...
        whitelist = [(path.split(os.sep)[0], path.split(os.sep)[1]) for path in config['whitelist']]
        ...
        train_seqs, train_labels = self.convert_instances(train_insts, seq_len=seq_len, behaviors=behaviors)
        test_seqs, test_labels = self.convert_instances(test_insts, seq_len=seq_len, behaviors=behaviors)

        return Dataset(train_seqs, train_labels, behaviors), Dataset(test_seqs, test_labels, behaviors)
...
def train_lstm_model(train_set, test_set, seq_len, behaviors, batch_size=512, lr=1e-4, epochs=10, device=None):
